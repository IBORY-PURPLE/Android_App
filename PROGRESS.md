...
**반복 횟수(검증 통과 커밋 기준): 25**

---

## 이터레이션 25 — 보정 액션 '나누기' (탭 분할선 + 위/아래 재크롭 — 5종 중 4호) (2026-07-18)

**한 일**
- 이터레이션 24의 다음 후보 ① 채택(②'박스 조절'은 4방향 핸들이라 더 무겁고, ③TSD 개정은 사람 승인 대기): **보정 액션 '나누기'(TSD.md 4.5 ② — 하나로 붙어 검출된 조각을 위/아래 둘로 분리)**, 삭제(19)·합치기(22)·순서 지정(23)에 이어 5종 중 4호. 남은 것은 박스 조절 하나.
- `SegmentationReviewPanel.tsx`: 후보 행에 **"나누기" 버튼**(안 뺀 조각만) → 그 조각을 크게 보여주는 **분할선 지정 화면** — 이미지를 **탭하면 가로 분할선이 그 자리로 이동**(기본 50%, 5~95% 클램프), "여기서 나누기"로 확정 / "나누지 않고 돌아가기"로 취소. **드래그·슬라이더 대신 탭인 이유:** RN 코어에 Slider가 없고(커뮤니티 패키지 = 새 의존성) 드래그는 PanResponder 과설계 — `Pressable`의 `locationY`/`onLayout`만으로 충분(채택 근거는 DECISIONS_NEEDED 18).
- 재크롭은 **합치기 인프라 그대로 재사용**(expo-image-manipulator `crop()` — 이터레이션 22 실측 API·같은 픽셀 변환 규칙·같은 JPEG 품질 0.9): 조각 bbox를 분할선 비율에서 수평으로 잘라 cleanedFull에서 두 사각형 재크롭, 위/아래 각각 최소 1px 보장. **위 조각은 원래 번호, 아래 조각은 새 번호(최대값+1)** — 확정 파일명 `line-<idx>.jpg`·행 key가 index 유일성을 요구(재번호 안 함 원칙과 일관 — 항목 12·15·16). 나뉜 조각은 원래 배열 자리에 위→아래로 들어가 표시 순서(→segment_order) 유지. 잘못 나누면 "위와 합치기"로 원탭 복구 또는 "자동 검출로 되돌리기"(un-split 스택은 과설계). `mergeError` → `actionError`로 이름 변경(합치기·나누기 실패를 같은 인라인 안내로 — 검토 작업 보존).
- 새 의존성 0, 새 Expo API 0(expo-image-manipulator 기존 실측 API + RN 기본 컴포넌트만 — docs 확인 대상 없음). 앱 본체 Expo Go 불변(패널은 개발 빌드 전용 경로 — 진입 전 흐름 변화 없음). 저장 계층 변경 0.
...
**커밋:** (이 커밋) feat: 보정 액션 '나누기' — 탭 분할선 + 위/아래 재크롭 (5종 중 4호)

**사람이 눈으로 볼 것 (실기 확인 필요):** 개발 빌드 — ① 줄 나누기 → 후보의 "나누기" → 분할선 지정 화면에서 탭으로 선이 움직이는지(가장자리 5% 클램프) ② "여기서 나누기" → 두 조각이 원래 자리에 위→아래로 들어가고 아래 조각이 새 번호(예: 1번을 나누면 1, N+1)인지 ③ 나눈 조각 확정 → '한 줄씩'에 화면 순서 그대로 뜨는지 ④ 나눈 두 조각을 "위와 합치기"로 도로 붙이기 / "자동 검출로 되돌리기" 복구 ⑤ 나눈 조각의 빼기·▲▼·재나누기(연쇄 분할)가 되는지. Expo Go — 앱 본체 평소대로(패널은 안내 후퇴), 콘솔 에러 0.

**다음 후보 (작은 순)**
1. 보정 액션 '박스 조절'(TSD.md 4.5 ③ — 5종 중 마지막) — bbox 가장자리 확대/축소. 핸들 드래그 대신 나누기와 같은 태도의 최소 설계(변 선택 + 늘리기/줄이기 스텝 버튼 등)부터 — 재크롭은 같은 crop 인프라.
2. '한 줄씩' 보기 다듬기(TSD.md 6.5) — 세로 목록 → 조각 스크럽(넘겨 보기)·핀치 줌은 무겁다면 조각 확대 표시부터.
3. TSD.md 5장 개정(react-native-android-widget 전제) + 6.3 processing_status 매핑(DECISIONS_NEEDED 9)·풀 편입 규칙(13)·미리보기 방식(14)·합치기 재크롭(15)·순서 지정(16)·목록 썸네일(17)·나누기(18) 명문화 — 사람 승인 후.
